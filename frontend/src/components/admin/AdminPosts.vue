            ...
            <tr v-for="post in posts" :key="post.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4">
                <div class="flex items-center">
                  <img :src="getAvatarUrl(post.author)" :alt="post.author?.username"
              ...
              <td class="px-6 py-4 text-sm font-medium space-x-2">
                <router-link :to="`/posts/${post.slug}`" class="text-blue-600 hover:text-blue-900" target="_blank">
                  查看
                </router-link>
                <router-link :to="`/edit-post/${post.id}`" class="text-green-600 hover:text-green-900">
                  编辑
                </router-link>
      ...
      <div v-if="totalPosts > limit" class="bg-white px-4 py-3 border-t border-gray-200 sm:px-6">
        <div class="flex items-center justify-between">
          <div class="text-sm text-gray-700">
            显示第 {{ (currentPage - 1) * limit + 1 }} 到 {{ Math.min(currentPage * limit, totalPosts) }} 条，共 {{ totalPosts
            }} 条
          </div>
          <div class="flex space-x-2">
            ...
            <button @click="loadPosts(currentPage + 1)" :disabled="!hasMore"
              class="px-3 py-1 border border-gray-300 rounded text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50">
              下一页
            </button>
          </div>
